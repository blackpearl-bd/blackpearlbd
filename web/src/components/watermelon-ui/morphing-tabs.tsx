      ...
      {active && surfaceHost && surfaceWidth > SURFACE_INSET * 2
        ? createPortal(
            <svg
              ...
              className={cn(
                "pointer-events-none absolute inset-x-0 top-0 h-[108px] w-full text-white dark:text-zinc-900",
                dragging ? "z-20" : "z-0",
                surfaceClassName,
              )}
            >
...
export function MorphingTabs({
  ...
  const { tabWidth, slotGap } = useMemo(() => {
    ...
    if (count > 1 && widthAt(0) >= MIN_TAB_WIDTH) {
      ...
      if (gap >= MIN_SLOT_GAP) {
        return { tabWidth: MIN_TAB_WIDTH, slotGap: gap };
      }
      // Gap too small — shrink tabs to ensure MIN_SLOT_GAP
      const adjusted = Math.floor(
        (inner - MIN_SLOT_GAP * (count - 1)) / count,
      );
      return { tabWidth: Math.max(0, adjusted), slotGap: MIN_SLOT_GAP };
    }
    // Very tight — shrink tabs to keep at least MIN_SLOT_GAP
    const adjusted = count > 1
      ? Math.floor((inner - MIN_SLOT_GAP * (count - 1)) / count)
      : inner;
    return { tabWidth: Math.max(0, adjusted), slotGap: MIN_SLOT_GAP };
  ...
  const slotLefts = useMemo(() => {
    const count = order.length;
    if (count <= 1) {
      return order.map(() => SURFACE_INSET);
    }
    // Justified layout: spread tabs evenly across the full width
    const totalAvailable = surfaceWidth - SURFACE_INSET * 2;
    const justifiedGap = Math.max(0, (totalAvailable - count * tabWidth) / (count - 1));
    return order.map(
      (_, index) => SURFACE_INSET + index * (tabWidth + justifiedGap),
    );
  }, [order, tabWidth, surfaceWidth]);
  ...
  const startDrag = useCallback(
    (id: string, event: ReactPointerEvent<HTMLDivElement>) => {
      ...
      let capturedSlots: number[];
      if (count <= 1) {
        capturedSlots = orderRef.current.map(() => SURFACE_INSET);
      } else {
        const totalAvailable = surfaceWidth - SURFACE_INSET * 2;
        const justifiedGap = Math.max(0, (totalAvailable - count * tabWidth) / (count - 1));
        capturedSlots = orderRef.current.map(
          (_, index) => SURFACE_INSET + index * (tabWidth + justifiedGap),
        );
      }
      ...
      };  }, [dragLeft, itemMap, slotGap, tabWidth, surfaceWidth],
  );
  ...
  return (
    <div
      ref={rootRef}
      className={cn(
        "relative isolate min-w-0 overflow-hidden rounded-2xl bg-zinc-900 text-white dark:bg-white dark:text-zinc-900",
        classNames?.root,
        className,
      )}
    >
                ...
                >
                    ...
                    <span
                      ...
                      className={cn(
                        ...
                        isDragging
                          ...
                          : "bg-transparent group-hover:bg-white/[0.06] dark:group-hover:bg-zinc-900/[0.06]",
                      )}
                    />
                  ) : null}
                  ...
                  >
                    <span
                      ...
                      className={cn(
                        ...
                        isActive
                          ? "bottom-0 rounded-t-[1.25rem] border-x-2 border-t-2 border-black/20 dark:border-white/20"
                          : "bottom-2 rounded-[1rem] border-2 border-white/60 dark:border-zinc-900/60",
                      )}
                    />
                    {item.icon ? (
                      <span
                  ...
                  {onClose ? (
                    <button
                      ...
                      className={cn(
                        "absolute right-2 top-1/2 z-20 grid size-6 -translate-y-1/2 place-items-center rounded-full text-zinc-500 transition-colors hover:bg-black/[0.06] hover:text-zinc-700 dark:text-zinc-400 dark:hover:bg-white/[0.06] dark:hover:text-zinc-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-current",
                        !isActive &&
                          "top-[calc(50%-4px)] text-white/45 hover:bg-white/[0.08] hover:text-white/80 dark:text-zinc-400 dark:hover:bg-zinc-900/[0.08] dark:hover:text-zinc-600",
                        classNames?.close,
                      )}
                    >
      ...
      <div
        ...
        className={cn(
          "relative z-20 mx-1 min-h-64 overflow-hidden rounded-2xl bg-background text-foreground dark:bg-zinc-900 dark:text-white",
          classNames?.content,
        )}
      >
